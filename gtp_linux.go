...
func (p *PDP) parseAttributes(attrs []syscall.NetlinkRouteAttr) error {
	for _, a := range attrs {
		...
		case nl.GENL_GTP_ATTR_VERSION:
			p.Version = nativeEndian.Uint32(a.Value)
		case nl.GENL_GTP_ATTR_TID:
			p.TID = nativeEndian.Uint64(a.Value)
		...
		case nl.GENL_GTP_ATTR_FLOW:
			p.Flow = nativeEndian.Uint16(a.Value)
		case nl.GENL_GTP_ATTR_NET_NS_FD:
			p.NetNSFD = nativeEndian.Uint32(a.Value)
		case nl.GENL_GTP_ATTR_I_TEI:
			p.ITEI = nativeEndian.Uint32(a.Value)
		case nl.GENL_GTP_ATTR_O_TEI:
			p.OTEI = nativeEndian.Uint32(a.Value)
		}
	}
	return nil
}
