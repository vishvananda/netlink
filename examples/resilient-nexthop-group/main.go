...
// Resilient nexhtop group creates a resilient group of nexthops
...
// ip nexthop add id 100 group 1/2 type res buckets 8 idle_timer 60 unbalanced_time 300
...
func main() {
	...
	group := make([]netlink.NexthopGroupMpath, 0, len(members))
	...
	for i, m := range members {
		...
		group = append(group, netlink.NexthopGroupMpath{
			ID:     id,
			Weight: m.weight,
		})
	}
...
func describeGroup(nh *netlink.Nexthop) string {
	...
	if nh.ResGroup != nil {
		desc += fmt.Sprintf(", Buckets: %d, IdleTimer: %d, UnbalancedTimer: %d, UnbalancedTime: %d",
			nh.ResGroup.Buckets, nh.ResGroup.IdleTimer, nh.ResGroup.UnbalancedTimer, nh.ResGroup.UnbalancedTime)
	}
	return desc
}
