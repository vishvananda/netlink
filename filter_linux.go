...
func (filter *Flower) ipv4FullMask() net.IPMask {
	return net.CIDRMask(32, 32)
}

func (filter *Flower) ipv6FullMask() net.IPMask {
...
func (filter *Flower) encodeIP(parent *nl.RtAttr, ip net.IP, mask net.IPMask, v4Type, v6Type int, v4MaskType, v6MaskType int) {
	...
	encodeMask := mask
	if mask == nil {
		encodeMask = filter.ipv4FullMask()
	}
	v4 := ip.To4()
	if v4 == nil && len(ip) == net.IPv6len {
		ipType = v6Type
		maskType = v6MaskType
		if mask == nil {
			encodeMask = filter.ipv6FullMask()
		}
	} else {
		ip = v4
	}
...
func (filter *Flower) bytesToIP(b []byte) net.IP {
	if len(b) == 4 {
...
func (filter *Flower) decode(data []syscall.NetlinkRouteAttr) error {
	for _, datum := range data {
		...
		case nl.TCA_FLOWER_KEY_IPV4_SRC, nl.TCA_FLOWER_KEY_IPV6_SRC:
			filter.SrcIP = filter.bytesToIP(datum.Value)
		...
		case nl.TCA_FLOWER_KEY_IPV4_DST, nl.TCA_FLOWER_KEY_IPV6_DST:
			filter.DestIP = filter.bytesToIP(datum.Value)
		...
		case nl.TCA_FLOWER_KEY_ENC_IPV4_SRC, nl.TCA_FLOWER_KEY_ENC_IPV6_SRC:
			filter.EncSrcIP = filter.bytesToIP(datum.Value)
		...
		case nl.TCA_FLOWER_KEY_ENC_IPV4_DST, nl.TCA_FLOWER_KEY_ENC_IPV6_DST:
			filter.EncDestIP = filter.bytesToIP(datum.Value)
